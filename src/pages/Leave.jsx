    ...
    try {
      ...
      if (stats) {
        setStats((s) => ({
          ...s,
          pending:
            status === "Pending" ? s.pending : Math.max(0, s.pending - 1),
          approvedThisMonth:
            status === "Approved"
              ? s.approvedThisMonth + 1
              : s.approvedThisMonth,
        }));
      }
      toast.success(
      ...
      <main style={{ flex: 1, position: "relative" }}>
        <AnimatePresence mode="wait">
                ...
                >
                  {[
                    ...
                    },
                    {
                      ...
                      value: loading ? "—" : (stats?.approvedThisMonth ?? 0),
          ...
          {activeTab === "calendar" && (
            <TabContent
              key="calendar"
              title={`Leave Calendar — ${MONTHS[now.getMonth()]} ${now.getFullYear()}`}
            >
              <div style={{ display: "grid", gap: "24px" }}>
                <div className="glass-card" style={{ padding: "24px" }}>
                  <div
            ...
            <TabContent key="balances" title="Leave Balances">
              <div style={{ display: "grid", gap: "24px" }}>
                <div className="glass-card" style={{ padding: "24px" }}>
                  ...
                  >
                    Policy-Wise Usage Summary
                  </h3>
                  <div style={{ display: "grid", gap: "12px" }}>
                    {(stats?.byType ?? []).map((t) => {
                      // default annual allocations
                      const allocated =
                        { Annual: 18, Sick: 10, Casual: 8, "Comp-Off": 4 }[
                          t.type
                        ] ?? 12;
                      const utilization = Math.min(
                        100,
                        Math.round((t.count / allocated) * 100),
                      );
                      return (
                        <div
                          key={t.type}
                    ...
                    {(stats?.byType ?? []).length === 0 && (
                      ...
                      >
                        No data yet — submit some leave requests first.
                      </p>
                    )}
                  </div>
                </div>
